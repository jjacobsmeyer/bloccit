class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*') # Select all attributes of the user
        ...
        .order('rank DESC') # Instructs the database to order the results in descending order, by the rank that we created in this query. (rank = coment count + post count)
  end
